...
NiceSelect.prototype.extractData = function () {
  ...
  options.forEach((item) => {
    var itemData = {
      text: item.innerText,
      value: item.value,
      icon: item.getAttribute('iconAttr'),
    };
  ...
  if (this.multiple) {
    ...
    var html =
      this.selectedOptions.length > 0
        ? this.selectedOptions[0].data.icon ? "<i class="+ this.selectedOptions[0].data.icon +"></i>" + this.selectedOptions[0].data.text : this.selectedOptions[0].data.text
        : this.placeholder;
...
NiceSelect.prototype._renderItem = function (option) {
  ...
  el.setAttribute('data-icon', option.data.icon);
  ...
  el.classList.add(...classList);
    if(option.data.icon) {
      el.innerHTML = '<i class="'+ option.data.icon +'"></i>';
    }
